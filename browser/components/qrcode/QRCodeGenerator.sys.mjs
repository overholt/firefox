...
export const QRCodeGenerator = {
  ...
  /**
   * Get or create the QRCode worker instance
   * @returns {QRCodeWorker} The worker instance
   */
  _getWorker() {
    if (!this._worker) {
      this._worker = new lazy.QRCodeWorker();
    }
    return this._worker;
  },
  ...
  async _loadFirefoxLogo(document) {
    ...
    return this._loadImage(document, "chrome://branding/content/about-logo.svg");
  },

  /**
   * Terminate the worker to free memory
   * Should be called when QR code generation is no longer needed
   */
  async cleanup() {
    if (this._worker) {
      try {
